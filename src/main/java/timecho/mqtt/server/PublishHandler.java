...
public class PublishHandler extends AbstractInterceptHandler {
  ...
  public PublishHandler() {
    this.payloadFormat = PayloadFormatManager.getPayloadFormat("json");
  }
  ...
  public void onConnect(InterceptConnectMessage msg) {
    LOG.info("open a connection ( iotdb will create a session) for {}", msg.getClientID());
  }

  @Override
  public void onDisconnect(InterceptDisconnectMessage msg) {
    LOG.info("close a connection ( iotdb will close a session) for {}", msg.getClientID());
  }
  ...
  public void onPublish(InterceptPublishMessage msg) {
    ...
    for (Message event : events) {
      ...
      LOG.info("event process result: {}", status);
    }
  }
}
